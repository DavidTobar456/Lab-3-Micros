...
PSECT code, delta = 2, abs
 ...
 clrf TRISD; Se configuran los pines ((EECON1) and 07Fh), 0 como salidas
 clrf TRISE; Se configuran los pines RE como salidas
 bsf TRISB,0;Se configura ((PORTB) and 07Fh), 0 como entrada
 bsf TRISB,1;Se configura ((PORTB) and 07Fh), 1 como entrada
 ...
 bcf ((OSCCON) and 07Fh), 4; Se configura el oscilador a 1MHz
 bsf ((OSCCON) and 07Fh), 0; Se selecciona el oscilador interno
    return

    config_timer0:
 banksel TRISA
 bcf ((OPTION_REG) and 07Fh), 5; El módulo TIMER0 corre con el reloj interno
 bcf ((OPTION_REG) and 07Fh), 3; El pre escalador se asigna a TIMER0
 bsf ((OPTION_REG) and 07Fh), 2
 bsf ((OPTION_REG) and 07Fh), 1
 bsf ((OPTION_REG) and 07Fh), 0; Se asignó el prescaler de 256 al timer
 banksel PORTA
 call reset_timer0
    return
